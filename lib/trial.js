...
// Called once from app.js after listen. Skipped when email isn't configured —
// there is nothing to send — and under tests.
function startTrialSweep() {
    if (timer) return;
    if (!process.env.RESEND_API_KEY) {
        console.warn("[trial] RESEND_API_KEY not set — trial emails disabled");
        return;
    }
    const run = () =>
        sweepTrialPrompts()
            .then((r) => {
                if (r.sent || r.failed) console.log(`[trial] sweep: ${JSON.stringify(r)}`);
            })
            .catch((error) => console.error("[trial] sweep failed:", error.message));
    setTimeout(run, FIRST_SWEEP_DELAY_MS).unref();
    timer = setInterval(run, SWEEP_INTERVAL_MS);
    timer.unref();
}

module.exports = { STAGES, sweepTrialPrompts, startTrialSweep, premiumUrl };
