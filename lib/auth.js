...
async function checkoutParamsFor({ user }) {
    try {
        const householdId = await db.getHouseholdIdForUser(user.id);
        if (!householdId) return {};
        const ent = await db.getEntitlement(householdId);
        if (ent.plan !== "trial" || !ent.trialEndsAt) return {};
        const endsAt = new Date(ent.trialEndsAt).getTime();
        if (endsAt - Date.now() < STRIPE_MIN_TRIAL_MS) return {};
        return { params: { subscription_data: { trial_end: Math.floor(endsAt / 1000) } } };
    } catch (error) {
