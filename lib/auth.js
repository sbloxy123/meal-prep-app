...
// Churn signals for the dashboard: cancelled (runs to period end) and ended.
function logSubscriptionEvent(type, subscription) {
    try {
        const started = subscription?.periodStart ? new Date(subscription.periodStart).getTime() : null;
        db.getHouseholdIdForUser(subscription?.referenceId).then((householdId) =>
            db.recordEvent(type, {
                userId: subscription?.referenceId ?? null,
                householdId,
                meta: {
                    interval: subscription?.billingInterval ?? null,
                    cancel_at_period_end: Boolean(subscription?.cancelAtPeriodEnd),
                    days_subscribed: started ? Math.round((Date.now() - started) / 86_400_000) : null,
                },
            }),
        );
    } catch (error) {
        console.error("[stripe] subscription event log failed:", error.message);
    }
}
        ...
        stripe({
            ...
            subscription: {
                ...
                onSubscriptionCancel: async ({ subscription }) => {
                    logSubscriptionEvent("subscription_cancelled", subscription);
                    return syncSubscriptionToHousehold(subscription);
                },
                onSubscriptionDeleted: async ({ subscription }) => {
                    logSubscriptionEvent("subscription_ended", subscription);
                    return syncSubscriptionToHousehold(subscription);
                },
            },
        }),
