...
//
//   const ledger = await openLedger({ householdId, userId, action, credits, planAt });
//   const message = await runModel(ledger, params);   // lib/ai.js records tokens/cost
//   await ledger.settle("ok");                         // or "refund" / "failed"
//
// The row is inserted 'pending' up front so in-flight work already counts
// against the allowance, then settled once. Outcomes:
...
async function openLedger({ householdId, userId = null, action, credits = 0, planAt = null, meta = null }) {
    let id = null;
    try {
        id = await db.insertAiUsage({ householdId, userId, action, credits, planAt, meta });
    } catch (error) {
        console.error("[ledger] open failed:", error.message);
    }
    return new Ledger({ id, action, credits });
}
...
module.exports = { openLedger, Ledger, errorCodeFrom };
